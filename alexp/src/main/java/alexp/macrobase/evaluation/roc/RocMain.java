...
import alexp.macrobase.evaluation.roc.util.CsvProcessing;
import alexp.macrobase.evaluation.roc.util.ArrayUtils;
import alexp.macrobase.evaluation.roc.util.NaiveCsvReader;
...
/**
 * <p>RocMain reads the results of a binary classification and generates a
 ...
 * generating ROC/PR analyses are easy.  Indeed, this RocMain will even
...
public class RocMain {
    ...
    public static final String help =
            "                      " + MetaInfo.NAME + ".  " + MetaInfo.TAGLINE + ".\n\n" +
                    ...
                    "java [JAVAOPTS] mloss.roc.RocMain [OPTION [ARGUMENT]]...\n\n" +
    ...
    /** Exception for RocMain that has an exit status. */
     ...
     */
    public static void RocMain(String[] args) {
        try {
            new RocMain().run(args);
            ...
            if (e instanceof RocMain.Exception) {
                exitStatus = ((RocMain.Exception) e).exitStatus.ordinal();
            }
    ...
    /** Constructs a RocMain with the given IO. */
    public RocMain(BufferedReader input,
                PrintWriter output,
                PrintWriter error) {
        this.input = input;
        this.output = output;
    ...
    /** Constructs a RocMain that uses standard IO. */
    public RocMain() {
    ...
    public void run(String[] args)
        ...
        for (int argIndex = 0; argIndex < args.length; argIndex++) {
            ...
            // Input files
                ...
                if (nextArgIndex >= args.length) {
                    throw new RocMain.Exception(String.format("File name missing after option: %s", arg), ExitStatus.ERROR_USAGE);
                }
                ...
                if (!fileName.equals(defaultFileName) &&
                        (!file.exists() ||
                                !file.isFile() ||
                                !file.canRead())) {
                    throw new RocMain.Exception(String.format("Not a readable file: %s", fileName), ExitStatus.ERROR_FILE);
                }
                // Store the file name
            ...
            // Integers
                ...
                if (nextArgIndex >= args.length) {
                    throw new RocMain.Exception(String.format("Integer missing after option: %s", arg), ExitStatus.ERROR_USAGE);
                }
                // Check the value is an integer
                String integerValue = args[nextArgIndex];
                if (!integerPattern.matcher(integerValue).matches()) {
                    throw new RocMain.Exception(String.format("Not an integer: %s", integerValue), ExitStatus.ERROR_USAGE);
                }
            ...
            // (Lists of) integers
                ...
                if (nextArgIndex >= args.length) {
                    throw new RocMain.Exception(String.format("Integer or list of integers missing after option: %s", arg), ExitStatus.ERROR_USAGE);
                }
                ...
                if (!integerListPattern.matcher(integerListValue).matches()) {
                    throw new RocMain.Exception(String.format("Not an integer or list of integers: %s", integerListValue), ExitStatus.ERROR_USAGE);
                }
                // Store the integer value
            ...
            // Strings and other unchecked/unparsed values
                ...
                if (nextArgIndex >= args.length) {
                    throw new RocMain.Exception(String.format("Argument missing after option: %s", arg), ExitStatus.ERROR_USAGE);
                }
                // Store the value
                ...
                // Report outputs
                    ...
                    if (reportGroups.size() == reportFiles.size()) {
                        throw new RocMain.Exception(String.format("No report options precede output file option: %s %s", reportFileOptName, args[nextArgIndex]), ExitStatus.ERROR_USAGE);
                    }
            ...
            // Unrecognized
                ...
                throw new RocMain.Exception(String.format("Unrecognized option: %s", arg), ExitStatus.ERROR_USAGE);
            }
        ...
        if (env.containsKey(scoresOptName) && !env.containsKey(labelsOptName)) {
            throw new RocMain.Exception(String.format("Option '%s' must be accompanied by option '%s'.", scoresOptName, labelsOptName), ExitStatus.ERROR_USAGE);
        }
        ...
        if (env.containsKey(scoresLabelsOptName)) {
            ...
            if (scoreCol == labelCol) {
                throw new RocMain.Exception(String.format("Column conflict for scores and labels: {%s} <-> {%s}", scoreCol + 1, labelCol + 1), ExitStatus.ERROR_USAGE);
            }
        ...
        // Input is scores and labels separately in CSV format
            ...
            if ((scoresKeyCols.length > 0 ||
                ...
                throw new RocMain.Exception("The same number of keys must be specified for both scores and labels.", ExitStatus.ERROR_USAGE);
            }
            ...
            if (ArrayUtils.indexOf(scoreCol, scoresKeyCols) != -1) {
                ...
                throw new RocMain.Exception(String.format("Column conflict for scores keys and scores: {%s} <-> {%s}", colsString, scoreCol + 1), ExitStatus.ERROR_USAGE);
            }
            if (ArrayUtils.indexOf(labelCol, labelsKeyCols) != -1) {
                String colsString = ArrayUtils.join(",",
                        ArrayUtils.toStrings(
                                ArrayUtils.add(1, labelsKeyCols)));
                throw new RocMain.Exception(String.format("Column conflict for labels keys and labels: {%s} <-> {%s}", colsString, labelCol + 1), ExitStatus.ERROR_USAGE);
            }
            ...
            // Run reports
                ...
                while (reportGroupsIter.hasNext() && reportFilesIter.hasNext()) {
                    ...
                    try {
                        ...
                        throw new RocMain.Exception(e.getMessage(), ExitStatus.ERROR_USAGE);
                    } finally {
    ...
    public static List<String[]> readCsv(
            ...
            String delimiter)
            throws IOException, RocMain.Exception {

        // Read the entire CSV
        List<String[]> csv = new NaiveCsvReader(input, delimiter).readAll();
        // Check for non-empty input
        if (csv.size() == 0) {
            throw new RocMain.Exception(String.format("Empty input: %s", fileName),
